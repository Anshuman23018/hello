...
public:
    ...
    Book(int book_id, int books_count, int isbn, const string& authors, const string& title, bool isElectronic = false)
...
public:
    ...
    BorrowedItem borrowedItems[4]; // Assuming a maximum of 4 borrowed items
    ...
    User(const string& userName, const string& userType) {
       ...
       void registerUser() {
        ...
        cout << "Enter your user type (Student/Faculty): ";
...
int main() {
    ...
    magazine_file.open("magazines.csv");
    ...
    journal_file.open("journals.csv");
    ...
    //    // cout << items[i]->getType() << " found with identifier " << items[i]->getIdentity() << endl;
    //     items[i]->display();
    //     cout << endl;
    // }

    // Create a user and demonstrate borrowing
    // User user("Hrishikesh", "Student");
    // User user("Saksham","Faculty");
    User user("","");
    ...
    cout << "Welcome " << user.userName << " (UserType): " << user.userType << endl;

while (true) {
    cout<<endl;
    cout << "What would you like to search for? "<<endl;
    cout<<"1 for Magazine"<<endl;
    cout<<"2 for Book"<<endl;
    cout<<"3 for Journal"<<endl;
    cout<<"4 for purchase a new book"<<endl;
    cout<<"5 to Exit:"<<endl;
    int searchChoice;
    cin >> searchChoice;
    ...
    switch (searchChoice) {
        ...
        case 2:
            ...
            foundBook = false;
            for (int i = 0; i < itemCount; i++) {
                if (items[i]->getType() == "Book" && items[i]->title == line) {
                    foundBook = true;
                    cout <<"Search in "<<items[i]->getType() << " section, find it on shelf: " << items[i]->getIdentity() << endl;
                    items[i]->display();
            ...
            if (!foundBook) {
                cout << "No book found with the title: " << line << "\n";
            }
            break;
