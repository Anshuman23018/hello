...
int readMagazinesFromFile(LibraryItem *items[], int &itemCount, int maxItems, const string &filename)
{
...
int readBooksFromFile(LibraryItem *items[], int &itemCount, int maxItems, const string &filename)
{
    ifstream book_file;
...
int readJournalsFromFile(LibraryItem *items[], int &itemCount, int maxItems, const string &filename)
{
...
int main()
{
    ...
    readBooksFromFile(items, itemCount, Max_books, "D:\\IIITD\\OOPD\\Assignment2\\books.csv");

    // Call the function to read magazines from a CSV file
    readMagazinesFromFile(items, itemCount, Max_books + Max_magazines, "D:\\IIITD\\OOPD\\Assignment2\\magazine.csv");

    // Call the function to read journals from a CSV file
    readJournalsFromFile(items, itemCount, Max_books + Max_magazines + Max_journals, "D:\\IIITD\\OOPD\\Assignment2\\journals.csv");
        ...
        case 3:
            ...
            for (int i = 0; i < itemCount; i++)
            {
                if (items[i]->getType() == "Journal" && items[i]->title == line)
                {
                    foundJournal = true;
                    items[i]->display();
                    cout << "Do you want to borrow this journal? (y/n): "<<endl;
                    char choice;
                    cin >> choice;
                    cin.ignore(); // Ignore the newline character
                    if (choice == 'y' || choice == 'Y')
