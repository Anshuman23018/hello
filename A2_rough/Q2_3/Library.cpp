...
int main()
{
    ...
    while (true)
    {
        ...
        cout << "Press 5 to Borrow a Book on loan from Other Libraries" << endl;
       ...
       case 5:
{
    cout << "Enter the title, author, or ISBN of the item you want to search in tie-up libraries: ";
    getline(cin, line);

    bool foundInTieUpLibrary = false;
    ...
    for (int i = 0; i < itemCount; i++)
    {
        if (items[i]->getType() == "Book")
        {
            ...
            if ((isISBN && line.length() == 13 && to_string(book->isbn) == line) ||
                book->title.find(line) != string::npos ||
                book->authors.find(line) != string::npos)
            {
                foundInTieUpLibrary = true;
                cout << "Item found in " << items[i]->getType() << " section, find it on shelf: " << items[i]->getIdentity() << endl;
                items[i]->display();

                cout << "From which library do you want to borrow this item?" << endl;
                for (int j = 0; j < librarySystem.getNumTieUps(); j++)
                {
                    cout << j + 1 << ". " << librarySystem.getTieUpLibrary(j)->getName() << endl;
                }
                cout << "Enter the library number (1/2/3/...): ";
                int libraryChoice;
                cin >> libraryChoice;
                cin.ignore(); // Ignore the newline character

                if (libraryChoice >= 1 && libraryChoice <= librarySystem.getNumTieUps())
                {
                    cout << "Do you want to borrow this item from " << librarySystem.getTieUpLibrary(libraryChoice - 1)->getName() << "? (y/n): ";
                    string borrowChoice;
                    getline(cin, borrowChoice);
                    if (borrowChoice == "y" || borrowChoice == "Y")
                    {
                        user.borrowOnLoan(librarySystem.getTieUpLibrary(libraryChoice - 1)->getItemByTitle(line), libraryChoice);
                    }
                    else
                    {
