...
void moneyReceived(Bank investments[],int numRows,float principal,int Year)
{
    ...
    cout<<"The Actual money received after "<<Year<<" years is: "<<newPrincipal<<endl;
}

void moneyInvested(Bank investments[], int numRows, float principal,int Year) {
    float value=principal;  // Initialize to a default value

    for(int i=0;i<numRows;i++) {
                if (investments[i].Year>=Year && investments[i].Year<=2022) {
            value=value*(1+(investments[i].Rate/100));
        }
    }
        cout<<"The investment money for year " <<Year<<" is: "<<value<<endl;
}
